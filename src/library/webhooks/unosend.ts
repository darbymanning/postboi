...
import { hmac_sha256, hex_encode, timing_safe_equal, whsec_key_candidates } from "./crypto.js"
...
const adapter: WebhookAdapter = {
	...
	async verify(ctx) {
		...
		const expected = await Promise.all(
			whsec_key_candidates(ctx.secret).map(async (key) =>
				hex_encode(await hmac_sha256(key, ctx.body))
			)
		)
