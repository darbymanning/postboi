...
one row covering both channels. **The SMS Works ships too, the other way round** — as a
`receive()` adapter (`webhooks/smsworks.ts`), because its delivery reports go to one
account-wide URL, which is exactly the shape every email webhook adapter already handles.
