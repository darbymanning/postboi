...
- **Push** — `to` is a Web Push subscription or a device token, so a target must be registered first: `subscribe()` from `postboi/push` in the browser (`sync` bakes in `VAPID_PUBLIC_KEY`, so it needs no options), `unsubscribe()` to drop it. For the settings-page toggle don't hand-roll the state machine — `subscription()` from `postboi/svelte` (runes: `push.on`, `push.busy`, `push.toggle`), `usePush` from `postboi/react`, `use_push` from `postboi/vue`, or `subscription()` from `postboi/push` (store contract) anywhere else. Providers: `webpush`, `fcm`, `apns`, `hms`. `/raw/push`
...
| Push toggle state machine            | `subscription` from `postboi/svelte` (runes) · `usePush` from `postboi/react` · `use_push` from `postboi/vue` · `subscription` from `postboi/push` anywhere else                             |
