...
| -------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------- |
| SMS      | one number or many                                                                             | `from`, `country` (ISO, resolves national numbers), `scheduled_at`, `tags`, `idempotency_key` | `twilio` `smsworks` `puresms` `sns`                  | `/raw/sms`                                                              |
| WhatsApp | number                                                                                         | `template` + `variables`, `header`, `buttons`, `language`                                     | `twilio` `meta`                                      | `/raw/whatsapp`                                                         |
| Push     | Web Push subscription or device token                                                          | `title`, `icon`, `url`, `urgency`, `ttl`                                                      | `webpush` `fcm` `apns` `hms` `expo`                  | `/raw/push`                                                             |
...
- **Expo / React Native apps get the same helper as `postboi/push/expo`** (over `expo-notifications`). It hands back `{ token, provider, platform }` — an `ExponentPushToken[…]` tagged `expo` for the `expo` server provider, which needs **no credential** but must be named (`POSTBOI_PUSH_PROVIDER=expo`; `EXPO_ACCESS_TOKEN` only under Enhanced Security, never inferred from), or with `{ native: true }` the raw token tagged `apns`/`fcm` for those providers. Store `provider` and pick the sender by it. Pass `storage: AsyncStorage` — the OS remembers permission, not registration, so without it the toggle forgets on restart. An app with a switch calls `subscribe()` when the user says yes and re-posts `subscribe.current()` on later launches (tokens rotate); only an app without a switch calls `subscribe()` every launch. `register` URLs must be absolute. On the server a 200 is a **ticket**, not a delivery — `new Expo().receipts(ids)` fetches the outcome, and `DeviceNotRegistered` on either is `push.expired()`.
- **Subscriptions rotate, and only the service worker hears about it.** `pushsubscriptionchange` fires nowhere else, so a worker without a handler misses one notification per rotation before the 410 self-heals it. `bunx postboi init --push` offers to wire the worker — it won't append to one that already handles `push` itself. The handler re-subscribes with the VAPID key and POSTs the replacement to your register endpoint with `old_endpoint` alongside it: delete that row, store the rest. `receive()`'s `key` also takes an async function, resolved only when a rotation needs to mint — for pairs that live server-side at runtime (a Workers secret behind the same endpoint the page subscribes off). And `click` takes over what a notification click does when the default (focus the exact-URL tab, or open one) isn't your app's shape — e.g. a single-window PWA navigating its one open tab.
...
bunx postboi segments create "Lapsed customers" --definition '{"match":"all","rules":[{"kind":"tag","tag":"customer","has":true},{"kind":"engagement","metric":"opened","within_days":60,"happened":false}]}'
bunx postboi segments "Lapsed customers"           # counts + who matches right now
bunx postboi sequences install welcome              # a shipped automation, as a draft
bunx postboi sequences simulate Welcome ada@example.com   # every step and what it would do — nothing sent
bunx postboi sequences enable Welcome              # or write sequences/welcome.ts and `postboi sync` pushes it
...
**Lifecycle (events and tags).** Every contact has a timeline; Postboi writes opens, clicks, bounces, replies and list changes to it, and the app adds its own with `mail.events.track(email | { external_id }, name, properties?)` — a signup, a purchase, a feature used. Use the documented names (`auth.signed_up`, `billing.trial_started`, `billing.purchase` with `amount` + `currency`, `billing.payment_failed`, `billing.churned`, `commerce.order_placed`; `mail.events.schemas()` lists them all) and snake_case custom names otherwise; the `email.*` / `contact.*` / `sms.*` / `whatsapp.*` prefixes are refused because Postboi records those. Wire `track` where the fact is already known — the auth signup handler, the Stripe webhook — and pass `{ idempotency_key }` from any handler that may retry. Tags (`mail.contacts.tag`) are current state; sync rules turn events into tags automatically, and every account starts with the SaaS preset (purchase → `customer`, churn → `churned`), so an app that reports billing events gets a segmentable audience for free. **Segments** are saved filters over the audience (`mail.segments.create(name, { match: "all" | "any", rules: [...] })`, predicates by `kind`: `tag`, `attribute`, `list`, `engagement`, `event`, `suppressed`, `phone`), evaluated fresh at every use — count them, page their contacts, broadcast to them (`mail.segments.broadcast`), or tag everyone in one. A broadcast reaches only matching contacts subscribed on some list; the response's `matching` vs `recipients` shows the gap. **Sequences** are automations — a trigger (`contact_added`, `tag_added`, `segment_entered`, `event`, `inactivity`, `frequency`, `manual`) and steps (`email`, `sms`, `whatsapp`, `push`, `slack`, `delay`, `wait_for_event`, `condition`, `branch`, `tag`, `list`, `webhook`, `exit`) a contact walks on their own clock. Write one as `sequences/<name>.ts` exporting `sequence({ name, trigger, steps })` (helpers: `sequence_email`, `delay`, `condition`, `tag`…) and `bunx postboi sync` pushes it as a draft; `mail.sequences.simulate(ref, { email })` shows every step without sending; `mail.sequences.test(ref, emails, 480)` runs it for named addresses with waits shrunk; `mail.sequences.enable(ref)` switches it on. Marketing steps skip contacts not subscribed on a list unless the step is `transactional: true`; suppressed addresses are never sent to. Full reference: https://docs.postboi.app/raw/provider#events--tags-the-timeline, https://docs.postboi.app/raw/provider#segments and https://docs.postboi.app/raw/provider#sequences.
...
| Expo / React Native push registration | `subscribe`, `unsubscribe`, `usePush`, `subscription` from `postboi/push/expo` — same names and reasons as the browser; `{ native: true }` for raw FCM/APNs tokens                                                                                                                       |
...
| Webhooks anywhere                     | `receive`, `handshake` (Meta's subscribe GET), `mock_event`, `mock_request` from `postboi/webhooks`                                                                                                                                                                                      |
