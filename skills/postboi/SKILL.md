...
- **`POSTBOI_*_PROVIDER` is optional, mostly.** A credential set that can only mean one provider is inferred, so a VAPID trio alone is a working Web Push setup. Name the provider when two providers' credentials are both present, and always for credentials the wider world sets for its own reasons — **SNS** (`AWS_*`), and every **chat** platform (`SLACK_WEBHOOK_URL` and friends are what CI notification actions set). For `send()`'s chat leg, a `to.chat` that is a recognisable Slack/Discord webhook URL names its own platform and needs nothing set.
