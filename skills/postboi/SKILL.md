...
- **`POSTBOI_*_PROVIDER` is optional.** A credential set that can only mean one provider is inferred, so a VAPID trio alone is a working Web Push setup. Name the provider only when two providers' credentials are both present.
