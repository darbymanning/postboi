...
- **Expo / React Native apps get the same helper as `postboi/push/expo`** (over `expo-notifications`; call `subscribe()` on every launch, tokens rotate). It hands back `{ token, provider, platform }` — an `ExponentPushToken[…]` tagged `expo` for the `expo` server provider, which needs **no credential** (`POSTBOI_PUSH_PROVIDER=expo`; `EXPO_ACCESS_TOKEN` only under Enhanced Security, and set alone it infers), or with `{ native: true }` the raw token tagged `apns`/`fcm` for those providers. Store `provider` and pick the sender by it. `register` URLs must be absolute; pass `storage: AsyncStorage` so the off switch survives a restart. On the server a 200 is a **ticket**, not a delivery — `new Expo().receipts(ids)` fetches the outcome, and `DeviceNotRegistered` on either is `push.expired()`.
